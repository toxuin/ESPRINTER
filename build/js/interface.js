...
function connect(password, firstConnect) {
	...
	$.ajax("rr_connect?password=" + password, {
		dataType: "json",
		error: function() {
 			showMessage("exclamation-sign", T("Error"), T("Could not establish connection to the 3D Printer!<br/><br/>Please check your settings and try again."), "md");
		...
		success: function(data) {
			if (data.err == 2) {		// Looks like the firmware ran out of HTTP sessions
				showMessage("exclamation-sign", T("Error"), T("Could not connect to 3D Printer, because there are no more HTTP sessions available."), "md");
				$("#modal_message").one("hide.bs.modal", function() {
					$(".btn-connect").removeClass("btn-warning disabled").addClass("btn-info").find("span:not(.glyphicon)").text(T("Connect"));
					$(".btn-connect span.glyphicon").removeClass("glyphicon-transfer").addClass("glyphicon-log-in");
				});
		...
		success: function(status) {
			...
			if (status.hasOwnProperty("output")) {
				...
				if (status.output.hasOwnProperty("message")) {
					showMessage("envelope", T("Message from 3D Printer"), status.output.message, "md");
				}
			}
...
function getConfigResponse() {
	$.ajax("rr_config", {
		...
		success: function(response) {
			...
			if (typeof response.firmwareDate != 'undefined') {
				$("#firmware_version").append(" (" + response.firmwareDate + ")");
			}
			if (typeof response.configFile != 'undefined') {
...
function resetGui() {
	...
	// Navbar
	$(".machine-name").html("3D Printer Web Control");
	...
	$("#div_config").html('<h1 class="text-center text-muted">' + T("Connect to your 3D Printer to display the configuration file") + '</h1>');
...
function clearGCodeFiles() {
	...
	if (isConnected) {
		...
		$("#page_files h1").text(T("Connect to your 3D Printer to display G-Code files"));
	}
}
...
function clearMacroFiles() {
	...
	if (isConnected) {
		...
		$("#page_macros h1").text(T("Connect to your 3D Printer to display Macro files"));
	}
}
...
function setGCodeFileItem(row, size, height, layerHeight, filamentUsage, generatedBy) {
	...
	var buttons =	'<button class="btn btn-success btn-print-file btn-sm" title="' + T("Print this file") + '"><span class="glyphicon glyphicon-print"></span></button>';
